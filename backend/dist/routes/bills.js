...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const { customerId, items, subtotal, taxRate, taxAmount, totalAmount, order_type, table_ref, order_label, } = req.body;
        ...
        if (order_type != null)
            extended.order_type = order_type;
        if (table_ref != null)
            extended.table_ref = table_ref;
        if (order_label != null)
            extended.order_label = order_label;
...
router.get('/stats/today', authMiddleware, async (req, res) => {
    try {
        ...
        const list = bills || [];
        const totalRevenue = list.reduce((sum, bill) => sum + Number(bill.total_amount || 0), 0);
      ...
      `)
        ...
        try {
            const { data: merchantData } = await supabase
                ...
                .eq('id', merchantId)
                .single();
            if (merchantData && merchantData.points_rate != null) {
                pointsRate = Number(merchantData.points_rate);
            }
        }
        catch {
        ...
        if (payment_last_four)
            updatePayload.payment_last_four = payment_last_four;
        ...
        if (bill.customer_id) {
            ...
            if (!custErr && customerRow) {
                ...
                const { data: activeCampaigns } = await supabase
                    ...
                    .order('created_at', { ascending: false })
                    .limit(4);
                const notificationResult = await sendNotification(c.email, c.name, Number(bill.total_amount), bill.merchants?.name || 'Restaurant', activeCampaigns || []);
                return res.json({
                    bill: finalBill,
