...
router.get('/stats', authMiddleware, async (req, res) => {
    try {
        ...
        const loyaltyRevenue = (bills || []).reduce((s, b) => s + Number(b.total_amount || 0), 0);
        ...
        const chart = DAYS.map((label, i) => {
            ...
            const revenue = dayBills.reduce((s, b) => s + Number(b.total_amount || 0), 0);
...
router.get('/tiers', authMiddleware, async (req, res) => {
    try {
        ...
        for (const c of customers || []) {
            const t = String(c.loyalty_tier || 'member').toLowerCase();
...
router.get('/streaks', authMiddleware, async (req, res) => {
    try {
        ...
        if (error)
            return res.status(400).json({ error: error.message });
        const streaks = (data || []).map((c, i) => ({
            id: c.id,
            name: c.name,
            tier: (c.loyalty_tier || 'member').replace(/^\w/, (x) => x.toUpperCase()) + ' Member',
...
router.get('/referrals', authMiddleware, async (_req, res) => {
    res.json({ newSignupsPct: 15, referralsCount: 154, changePct: 12 });
});
router.get('/cashback', authMiddleware, async (_req, res) => {
