...
router.get('/profile', authMiddleware, async (req, res) => {
    try {
        ...
        res.json({
            ...data,
            tax_rate: data.tax_rate ?? 0.085,
            receipt_template: data.receipt_template || 'standard',
            staff_roles: data.staff_roles || ['admin', 'manager', 'cashier'],
            points_rate: data.points_rate ?? 5,
        });
    }
    catch (error) {
        res.status(500).json({ error: 'Internal server error' });
...
router.put('/profile', authMiddleware, async (req, res) => {
    try {
        ...
        if (tax_rate !== undefined)
            extraPayload.tax_rate = Number(tax_rate);
        if (receipt_template !== undefined)
            extraPayload.receipt_template = receipt_template;
        if (points_rate !== undefined)
            extraPayload.points_rate = Number(points_rate);
        if (staff_roles !== undefined)
            extraPayload.staff_roles = Array.isArray(staff_roles)
                ? staff_roles
                : String(staff_roles)
                    .split(',')
                    .map((role) => role.trim())
        ...
        res.json({
            ...data,
            tax_rate: data.tax_rate ?? tax_rate ?? 0.085,
            receipt_template: data.receipt_template || receipt_template || 'standard',
            points_rate: data.points_rate ?? points_rate ?? 5,
            staff_roles: data.staff_roles || (Array.isArray(staff_roles)
                ? staff_roles
