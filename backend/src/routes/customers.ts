...
router.get('/search/:phone', authMiddleware, async (req: Request, res: Response) => {
  try {
    const merchantId = req.merchantId;
...
router.post('/', authMiddleware, async (req: Request, res: Response) => {
  try {
    const merchantId = req.merchantId;
    const { name, phone, email, telegram_chat_id } = req.body;
...
function inferStatus(c: any): string {
  if ((c.total_spend || 0) >= 3000) return 'vip';
  if ((c.total_visits || 0) <= 1) return 'new';
  if ((c.total_spend || 0) <= 150 && (c.total_visits || 0) >= 2) return 'at_risk';
  return 'active';
}
...
router.post('/:id/message', authMiddleware, async (req: Request, res: Response) => {
  try {
    const merchantId = req.merchantId;
    const { id } = req.params;
...
router.get('/:id/top-items', authMiddleware, async (req: Request, res: Response) => {
  try {
    const merchantId = req.merchantId;
    const { id } = req.params;
    ...
    for (const bill of bills || []) {
      const items = (bill as any).items as Array<{ item_name?: string; quantity?: number }> | null;
...
router.put('/:id', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    if (points_balance !== undefined) patch.points_balance = points_balance;
    if (email !== undefined) patch.email = email;
    if (crm_status !== undefined) patch.crm_status = crm_status;
    if (loyalty_tier !== undefined) patch.loyalty_tier = loyalty_tier;
